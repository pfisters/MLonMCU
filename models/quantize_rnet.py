...
flags.DEFINE_string('pnet_weights', './models/rnet.h5',
    'path to the weights of the RNet')
flags.DEFINE_string('pnet_light', './models/rnet.tflite',
...
def main(args):
    ...
    if FLAGS.micro_data:
        ...
        np.savetxt('rnet_cat.csv', v_cat.reshape(FLAGS.validation_size,-1), delimiter=',')
        np.savetxt('rnet_bbx.csv', v_bbx.reshape(FLAGS.validation_size,-1), delimiter=',')
    ...
    if FLAGS.save_inputs:
        filename = 'rnet_test cat|' + '|'.join(map(str,cat_output.flatten())) + '|bbx|' + '|'.join(map(str, bbx_output.flatten()))
